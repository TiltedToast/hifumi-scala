...
final class ReadyListener extends ListenerAdapter {
    override def onReady(event: ReadyEvent): Unit = {
        ...
        val timeDiff = Duration.between(startupTime, readyTime).toMillis / 1000.0
...
final class MsgListener extends ListenerAdapter {
    override def onMessageReceived(event: MessageReceivedEvent): Unit = {
        ...
        if !content.head.startsWith(prefix) then return

        content.head.toLowerCase().stripPrefix(prefix) match {
